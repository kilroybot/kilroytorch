<h1 align="center">kilroytorch</h1>
...
For more info about the package itself see `kilroytorch/README.md`
or [docs](https://kilroybot.github.io/kilroytorch).
...
(base) $ git clone https://github.com/kilroybot/kilroytorch
(base) $ cd kilroytorch
(base) $ conda env create -f environment.yml
(base) $ conda activate kilroytorch
(kilroytorch) $ cd kilroytorch
(kilroytorch) $ poetry install --extras dev
```
...
```sh
$ python3 -m pip install ./kilroytorch
```
...
To install the package, you need to `cd` into `kilroytorch` directory and run:
...
write the tests and put them in `kilroytorch/tests`.
...
Docs should be placed in `kilroytorch/docs/docs`. They are pretty
straightforward to write.

To build the docs, `cd` into `kilroytorch/docs` and run:
...
After that update the installation by running from `kilroytorch` directory:
